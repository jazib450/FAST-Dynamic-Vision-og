...
void DepthEst::main(const sensor_msgs::ImageConstPtr& msg) {
  ...
  //ROS_INFO_STREAM("k_depth_camera_intrinsic_: \n" << k_depth_camera_intrinsic_);
  //ROS_INFO_STREAM("k_event_camera_intrinsic_: \n" << k_event_camera_intrinsic_);
  //ROS_INFO_STREAM("k_distort_coeff_: \n" << k_distort_coeff_);
  //ROS_INFO_STREAM("k_RT_event2depth_: \n" << k_RT_event2depth_);

  // Debugging part to check mean and standard deviation values
  //cv::Scalar mean_val, stddev_val;
  //cv::meanStdDev(depth_gray_, mean_val, stddev_val);
  //ROS_INFO("Mean: %f, StdDev: %f", mean_val[0], stddev_val[0]);
   ...
   * detection will be regarded as invalid
   */
  valid_count_++;
  if (valid_count_ > k_valid_frame_) {
    is_obj_ = false;
  }
  //ROS_INFO("Object detection flag (is_obj_): %s", is_obj_ ? "True" : "False");
  ...
  if (is_obj_) {  
    ...
    if (obj_depth_flag_) {
      ...
      if ((std[0] < 100) && (std[0] > 0)) {
        ...
        /* if detections on depth image are too close to image edge, 
